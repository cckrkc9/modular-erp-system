...
public class FinancialRecordService {
    ...
    public List<FinancialRecord> getFinancialRecordsByType(String type)
    {
        return StreamSupport.stream(m_financialRecordRepository.findByType(FinancialRecordType.valueOf(type)).spliterator(), false)
                .collect(Collectors.toList());
    }

    public List<FinancialRecord> findFinancialRecordsLessThan(BigDecimal amount, FinancialRecordType type)
    {
        return StreamSupport.stream(m_financialRecordRepository.findLessThan(amount, type).spliterator(), false)
                .collect(Collectors.toList());
    }
}